...
?>
<?php
	$featured = skyMysqlQuery('select id,file_name,category_id,singer,size,download,extension,created_at from files where status!="B" order by id desc limit '.SETTING_LATEST_FILES);
	myUser::getc('RmVhdHVyZWQgVmlkZW9z',1);
?>
<div class="featured">
  <h2>Latest Update</h2>
<?php
...
if($files->singer)
    		echo '<span class="mc">Singer: </span> <span class="alb"> '.str_replace(',',', ',substr($files->singer,1,-1)).'</span><br/>';
...
<?php endwhile; ?>
  ...
  <div>
  	<?php echo link_to('[More...]','@lastAddedFiles', array('class'=>'')) ?>
  </div>
</div>
