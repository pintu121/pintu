...
//				$files->setTags($parentCategory->getCategoryName().', '.myUser::fileName($saveAs,false));
		...
		*/
		...
		if($files->getExtension()=='MP3' && $this->getRequestParameter('mp3tags')=='true' && ($this->getRequestParameter('url_path') || $this->getRequest()->getFileSize('file_name') > 0) ){
			header('Location: /ADmiN/SkyWriteMp3.php?Filename='.$dataServer.'/'.$files->getId().'/'.$files->getFileName().'&cid='.$files->getCategoryId().'&singer='.base64_encode(str_replace(',',' ,',$this->getRequestParameter('singer'))).'&catName='.base64_encode(CategoryPeer::getCategoryName($files->getCategoryId())).'&lastFile='.base64_encode($files->getFileName()) );
			exit;
		}
		else
  ...
  public function executeActivation()
  {
    ...
    $this->files->save();
  	if($this->getRequestParameter('status') == 'B' || $this->getRequestParameter('status') == 'F')
  		echo '<a onclick="new Ajax.Updater(\'active_'.$this->getRequestParameter('id').'\', \'/ADmiN/files/activation/id/'.$this->getRequestParameter('id').'/status/A\', {asynchronous:true, evalScripts:false});; return false;" href="#">Active</a>';
  	else
  		echo '<a onclick="new Ajax.Updater(\'active_'.$this->getRequestParameter('id').'\', \'/ADmiN/files/activation/id/'.$this->getRequestParameter('id').'/status/B\', {asynchronous:true, evalScripts:false});; return false;" href="#"><span class="red">Block</span></a>';
		return sfView::NONE;
  }
